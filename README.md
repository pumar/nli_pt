...
An additional preprocessing step consisted of using a token () for names instead of the proposed "XXXXX" that replaced proper names in the original dataset. While fine for a word-level tokenization, unnecessary repetition should be avoided when applying character level ngrams.
